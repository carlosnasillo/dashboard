...
object PortfolioAnalytics {
  ...
  private[impl] def pendingInvestment(notes: Seq[LendingClubNote]) = notes.collect { case x if LoanStatus.isPending(x.loanStatusEnum) => x.noteAmount }.sum
  private[impl] def paymentsReceived(notes: Seq[LendingClubNote]) = notes.map(_.paymentsReceived).sum
  private[impl] def principalReceived(notes: Seq[LendingClubNote]) = notes.map(_.principalReceived).sum
  ...
  private[impl] def notesAcquiredToday(notesByDate: Map[LocalDate, Seq[LendingClubNote]]) = notesAcquired(notesByDate, LocalDate.now, LocalDate.now).getOrElse(LocalDate.now,0)
  private[impl] def notesAcquiredTodayByGrade(notesByDate: Map[LocalDate, Seq[LendingClubNote]]) = notesAcquiredByGrade(notesByDate, LocalDate.now, LocalDate.now).getOrElse(LocalDate.now,Map())
  private[impl] def notesAcquiredTodayByYield(notesByDate: Map[LocalDate, Seq[LendingClubNote]]) = notesAcquiredByYield(notesByDate, LocalDate.now, LocalDate.now).getOrElse(LocalDate.now,Map())
  private[impl] def notesAcquiredTodayByPurpose(notesByDate: Map[LocalDate, Seq[LendingClubNote]]) = notesAcquiredByPurpose(notesByDate, LocalDate.now, LocalDate.now).getOrElse(LocalDate.now,Map())
  ...
  def analyse(notes: Seq[LendingClubNote]): PortfolioAnalytics = {
    PortfolioAnalytics(principalOutstanding(notes),
      ...
      paymentsReceived(notes),
      principalReceived(notes),
...
case class PortfolioAnalytics(
    ...
    override val paymentsReceived: BigDecimal,
    override val principalReceived: BigDecimal,
